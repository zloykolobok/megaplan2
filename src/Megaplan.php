...
use Zloykolobok\Megaplan2\SdfApi_Request;
use Zloykolobok\Megaplan2\Exception\ConnectionException;
...
class Megaplan
{
    ...
    public function auth()
    {
        ...
        if(!$host){
            throw new ConnectionException('Не установлен хост');
        }

        if($api){
            if(!$login){
                throw new ConnectionException('Не установлен логин для доступа к мегаплану');
            }
            if(!$password){
                throw new ConnectionException('Не установлен пароль для доступа к мегаплану');
            }
        } else {
            if(!$accessId){
                throw new ConnectionException('Не установлен UUID приложения для доступа к мегаплану');
            }
            if(!$secretKey){
                throw new ConnectionException('Не установлен API ключ для доступа к мегаплану');
            }
        }

        if($api) {
            // Авторизуемся в Мегаплане
            $this->req = new SdfApi_Request( '', '', $host, $https );
